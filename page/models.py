...
# tutorial to add web forms to the pages. Not a stream form but who cares https://stackoverflow.com/questions/48636619/wagtail-feedback-form-in-homepage
    ...
    content = StreamField([
        ...
        ('events_by_category', MultipleChoiceBlock(choices=get_categories,widget=forms.CheckboxSelectMultiple, required=False, help_text="If checked all upcoming programs filtered by a category will display", icon='tasks')),
        ('events_by_audience', MultipleChoiceBlock(choices=get_audiences,widget=forms.CheckboxSelectMultiple, required=False, help_text="If checked all upcoming programs filtered by a audiences will display", icon='tasks')),
        ('DateBlock', DateBlock()),
        ('TimeBlock', TimeBlock()),
